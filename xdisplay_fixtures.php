<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <title>Fixtures Dashboard</title>
    <style>
        body {
            font-family: Arial, sans-serif;
            margin: 20px;
        }
        table {
            width: 100%;
            border-collapse: collapse;
            margin-top: 20px;
        }
        table, th, td {
            border: 1px solid #ccc;
        }
        th, td {
            padding: 10px;
            text-align: left;
        }
        .pool {
            margin-bottom: 30px;
        }
        h2 {
            margin-top: 0;
        }
    </style>
</head>
<body>
    <h1>Fixtures Dashboard</h1>

    <div class="pool">
        <h2>Pool A Fixtures</h2>
        <table>
            <tr>
                <th>Player 1</th>
                <th>Player 2</th>
                <th>Round</th>
                <th>Winner</th>
            </tr>
            <?php
            require_once 'config.php';
            $result = mysqli_query($conn, "SELECT f.round, p1.player_name AS player1, p2.player_name AS player2, w.player_name AS winner
                                           FROM fixtures f
                                           LEFT JOIN players p1 ON f.player1_id = p1.id
                                           LEFT JOIN players p2 ON f.player2_id = p2.id
                                           LEFT JOIN players w ON f.winner_id = w.id
                                           WHERE f.pool = 'A'");
            while ($row = mysqli_fetch_assoc($result)) {
                echo "<tr>
                        <td>{$row['player1']}</td>
                        <td>{$row['player2']}</td>
                        <td>{$row['round']}</td>
                        <td>{$row['winner']}</td>
                      </tr>";
            }
            ?>
        </table>
    </div>

    <div class="pool">
        <h2>Pool B Fixtures</h2>
        <table>
            <tr>
                <th>Player 1</th>
                <th>Player 2</th>
                <th>Round</th>
                <th>Winner</th>
            </tr>
            <?php
            $result = mysqli_query($conn, "SELECT f.round, p1.player_name AS player1, p2.player_name AS player2, w.player_name AS winner
                                           FROM fixtures f
                                           LEFT JOIN players p1 ON f.player1_id = p1.id
                                           LEFT JOIN players p2 ON f.player2_id = p2.id
                                           LEFT JOIN players w ON f.winner_id = w.id
                                           WHERE f.pool = 'B'");
            while ($row = mys
